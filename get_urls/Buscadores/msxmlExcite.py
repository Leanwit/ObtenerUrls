...
def generar_consulta_excite(consultas):
    ...
    for consulta in consultas:
        ...
        try:
                element = WebDriverWait(driver, 50).until(EC.presence_of_element_located((By.CSS_SELECTOR, "div.searchResult")))
        finally:
        ...
        driver.maximize_window()
        consulta = consulta.replace(" ","+")
        driver.get("http://msxml.excite.com/info.xcite/search/web?fcoid=417&fcop=topnav&p=2&fpid=27&q="+str(consulta))
        try:
                element = WebDriverWait(driver, 50).until(EC.presence_of_element_located((By.CSS_SELECTOR, "div.searchResult")))
        finally:
            ids = driver.find_element_by_id('resultsMain')
	...
	urls = eliminarDuplicados(urls)	
    return urls


def eliminarDuplicados(urls):
	urlsDefinitivos = []
	for unaUrl in urls:
	    if not unaUrl in urlsDefinitivos:
		urlsDefinitivos.append(unaUrl)
	return urlsDefinitivos
